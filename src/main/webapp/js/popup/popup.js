...
function jqalert(param) {
    ...
    if (prompt){
        htm += '<div class="cont"><div class="prompt">';
        htm += '<p class="prompt-content">'+prompt+'</p>';
        htm += '<input type="text" class="prompt-text"></div>';
        htm +='</div>';
    }else {
        htm+='<div class="cont">'+cont+'</div>';
    }
    if (!notext){
        htm+='<div class="fd-btn"><button class="confirm btn btn-primary" id="yes_btn">'+yestext+'</button></div>'
        htm+='</div>';
    }else {
        htm+='<div class="fd-btn">'+
            '<button data-role="cancel" class="btn btn-primary" style="margin-right:10px;">'+notext+'</button>'+
            '<button class="btn btn-primary"  id="yes_btn" style="margin-right:-240px;">'+yestext+'</button>'+
            '</div>';
        htm+='</div>';
    }
    $('body').append(htm);
